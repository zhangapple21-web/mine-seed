...
CLOUD_DIR = WORKSPACE / "cloud" / "archivist"


def run_archivist():
    CLOUD_DIR.mkdir(parents=True, exist_ok=True)
    output_file = CLOUD_DIR / f"archivist_{datetime.now().strftime('%Y%m%d')}.json"

    try:
        from memory.archivist import archive_workspace
        report = archive_workspace()
        with open(output_file, "w", encoding="utf-8") as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
        return {"status": "ok", "output": str(output_file)}
...
def git_push():
    try:
        ...
        subprocess.run(["git", "commit", "-m", f"cloud: archivist {datetime.now().strftime('%Y%m%d')}"],
...
def main():
    ...
    if args.notify:
        notify_ntfy(f"Archivist: {result.get('status')}")
