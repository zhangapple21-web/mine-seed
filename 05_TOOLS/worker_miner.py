...
CLOUD_DIR = WORKSPACE / "cloud" / "miner"


def run_miner():
    CLOUD_DIR.mkdir(parents=True, exist_ok=True)
    output_file = CLOUD_DIR / f"miner_{datetime.now().strftime('%Y%m%d')}.json"

    try:
        from miner.task_router_v2 import run_miner_tasks
        result = run_miner_tasks()
...
def git_push():
    try:
        ...
        subprocess.run(["git", "commit", "-m", f"cloud: miner {datetime.now().strftime('%Y%m%d')}"],
...
def main():
    parser = argparse.ArgumentParser(description="Miner Worker")
    ...
    if args.notify:
        notify_ntfy(f"Miner: {result.get('status')}")
