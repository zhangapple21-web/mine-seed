...
## 核心定理（2026-08-25 边界修订）

> **系统不会停止观察，但可以没有值得执行的任务。**

Work 不是因为人派单或资源空闲而产生。它从 Reality 中被发现，并且只有在来源、证据、价值和边界通过评估后才成为 Task。
...
系统活着意味着观察和连续性责任持续存在，不意味着 TaskPool 必须始终非空。`Idle / Watch` 不是等待用户派单，而是有界发现后的合法状态。
...
- "等待是异常" = 自治循环不依赖用户派单；不否定证据驱动的 Idle / Watch
