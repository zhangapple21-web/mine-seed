...
| 模块 | 功能 | 激活方式 |
|------|------|----------|
| M01 | 信号验证 | 按需激活（有新信号待验证） |
| M02 | 失效分析 | 按需激活（Worker失效时） |
| M03 | Worker画像 | 按需激活（Worker注册/更新时） |
| M04 | Constraint提案 | 按需激活（发现新约束需求时） |
| M05 | 同构检测 | 按需激活（新资产入库时） |
| M06 | R1考古 | 按需激活（发现新碎片时） |
...
| M01 信号验证 | 按需激活 | 有新信号待验证 | ✅ 已定义 |
| M02 失效分析 | 按需激活 | Worker失效时 | ✅ 已定义 |
| M03 Worker画像 | 按需激活 | Worker注册/更新时 | ✅ 已定义 |
| M04 Constraint提案 | 按需激活 | 发现新约束需求时 | ✅ 已定义 |
| M05 同构检测 | 按需激活 | 新资产入库时 | ✅ 已定义 |
| M06 R1考古 | 按需激活 | 发现新碎片时 | ✅ 已定义 |
| M4 边界守卫 | 永不解锁 | — | ✅ 已定义（READ_ONLY） |

---

*机制已建立。考古发现的种子不会丢失，只等待条件成熟时发芽。M4 守护边界，其他 M 按需解放。*